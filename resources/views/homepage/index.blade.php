...
@section('content')
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-lg-6 px-5">
          <img src="{{ asset('storage/' . $setting->foto_tentang_kami) }}" class="img-fluid" alt="">
        </div>
        ...
        @foreach ($products as $index => $product)
          <div class="col-lg-4 col-md-6 produk-item filter-morando wow fadeInUp" data-wow-delay="0.1s">
            <div class="produk-wrap text-center">
              <a href="detail-produk/{{ $product->id }}" class="link-details" title="Lihat Detail">
                <figure>
              ...
              <div class="produk-info">
                <h4><a href="detail-produk/{{ $product->id }}">{{ $product->nama }}</a></h4>
                <p id="harga-{{ $index }}">{{ $product->harga }}</p>
              </div>
            </div>
          </div>
        @endforeach
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      @foreach ($products as $index => $product)
        var hargaElement = document.getElementById('harga-{{ $index }}');
        var harga = hargaElement.innerText;

        var rupiah = new Intl.NumberFormat("id", {
          style: "currency",
          currency: "IDR",
